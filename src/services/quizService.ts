...
export const addQuiz = async (quizData: Omit<Quiz, "_id" | "createdAt" | "updatedAt">) => {
  const { data } = await api.post("/quizzes", quizData);
  return data;
};

export const updateQuiz = async (id: string, quizData: Omit<Quiz, "_id" | "createdAt" | "updatedAt">) => {
