...
const selectedTopicForRetake = ref<{ topicId: string; courseId: string } | null>(null);
  ...
  <div class="mb-4">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <Card
        ...
        class="shadow-md"
      >
        <template #title> 📚 {{ topic.topicTitle }} </template>
        <template #content>
          <p class="mb-2">
          ...
          <div class="mt-3 flex justify-between">
            <Badge
              ...
              severity="success"
            />
            <Badge v-else value="In Progress" severity="warning" />
            <Button
              v-if="!topic.isCompleted"
              label="🔄 Retake Quiz"
              severity="info"
